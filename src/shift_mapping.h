...
ReservoirPTSample reconnection_shift_mapping(const Scene &scene, ReservoirPT source_reservoir, ReservoirPT target_reservoir){
    ...
    PathVertex q = *target_reservoir.org_vertex;
    PathVertex reconn_vertex = *path_to_shift.reconnection_vertex;
    // Check visibility
    Ray shadow_ray{q.position, normalize(reconn_vertex.position - q.position), get_shadow_epsilon(scene), (1-get_shadow_epsilon(scene)) * distance(reconn_vertex.position, q.position)};
    if (occluded(scene, shadow_ray)){
        return ReservoirPTSample();
    }

    const Material &mat = scene.materials[reconn_vertex.material_id];
    Vector3 dir_view = normalize(q.position - reconn_vertex.position);

    // Re-calculate radiance for the reconnection vertex
    Spectrum radiance = make_zero_spectrum();
    // If is light, account for the emission
    if (is_light(scene.shapes[reconn_vertex.shape_id])){
        radiance += emission(reconn_vertex, dir_view, scene);
    }
    ...
    PointAndNormal point_on_light = sample_point_on_light(light, reconn_vertex.position, light_uv, shape_w, scene);
    ...
    Real w1 = 0;
    {
        ...
        if(!is_envmap(light)){
            dir_light = normalize(point_on_light.position - reconn_vertex.position);

            // Check visibility
            Ray shadow_ray{reconn_vertex.position, dir_light, get_shadow_epsilon(scene),
                        (1-get_shadow_epsilon(scene) * distance(point_on_light.position, reconn_vertex.position))};

            if (!occluded(scene, shadow_ray)) {
                G = max(-dot(dir_light, point_on_light.normal), Real(0)) / 
                distance_squared(point_on_light.position, reconn_vertex.position);
            }
        } else {
            dir_light = -point_on_light.normal;

            Ray shadow_ray{reconn_vertex.position, dir_light,
        ...
        Real p1 = light_pmf(scene, light_id) * pdf_point_on_light(light, point_on_light, reconn_vertex.position, scene);

        if (G > 0 && p1 > 0){
            assert(reconn_vertex.material_id >= 0);
            Spectrum f = eval(mat, dir_view, dir_light, reconn_vertex, scene.texture_pool);

            Spectrum L = emission(light, -dir_light, Real(0), point_on_light, scene);

            C1 = G * f * L;

            Real p2 = pdf_sample_bsdf(mat, dir_view, dir_light, reconn_vertex, scene.texture_pool);
    ...
    if (path_to_shift.recon_out_dir.x != 0 || path_to_shift.recon_out_dir.y != 0 || path_to_shift.recon_out_dir.z != 0){
        Ray bsdf_ray{reconn_vertex.position, path_to_shift.recon_out_dir, get_intersection_epsilon(scene), infinity<Real>()};
        ...
        if (bsdf_vertex){
            ...
            distance_squared(bsdf_vertex->position, reconn_vertex.position);
        } else {
            G = 1;
        }

        Spectrum f = eval(mat, dir_view, path_to_shift.recon_out_dir, reconn_vertex, scene.texture_pool);
        Real p2 = pdf_sample_bsdf(mat, dir_view, path_to_shift.recon_out_dir, reconn_vertex, scene.texture_pool);
        if (p2 >0) {
            ...
            if (!bsdf_vertex && has_envmap(scene)){
                ...
                Real p1 = light_pmf(scene, scene.envmap_light_id) * pdf_point_on_light(light, light_point, reconn_vertex.position, scene);
                ...
                if (is_light(scene.shapes[bsdf_vertex->shape_id])){
                    ...
                    Real p1 = light_pmf(scene, light_id) * pdf_point_on_light(scene.lights[light_id], 
                        PointAndNormal{bsdf_vertex->position, bsdf_vertex->geometric_normal}, reconn_vertex.position, scene);
    ...
    Spectrum f = eval(scene.materials[q.material_id], normalize(cam_org - q.position), -dir_view, q, scene.texture_pool);
    Real G = fabs(dot(dir_view, reconn_vertex.geometric_normal)) /
    distance_squared(reconn_vertex.position, q.position);
...
std::tuple<Real, Real> MIS(const Scene &scene, ReservoirPT& source_reservoir, ReservoirPT& target_reservoir, ReservoirPTSample& shifted_sample, ReservoirPTSample& reverse_shifted_sample){
    ...
    Real jacobian_det = fabs(dot(normalize(x1.position - source_reservoir.org_vertex->position), x1.geometric_normal) / 
                            dot(normalize(x1.position - x.position), x1.geometric_normal)) * distance_squared(x.position, x1.position) /
                            distance_squared(x1.position, source_reservoir.org_vertex->position);
    Real p_hat = y.p_hat;
    Real p_hat_j = inverse_y.p_hat * jacobian_det; // Modify this to multiply with determinant of jacobian
    Real target_mis = 0;
    if (R > 0){
        target_mis = 1/(M_R) * (p_hat * R) / (p_hat * R + p_hat_j * M_R);
    }
    ...
    jacobian_det = fabs(dot(normalize(x1.position - source_reservoir.org_vertex->position), x1.geometric_normal) / 
                            dot(normalize(x1.position - x.position), x1.geometric_normal)) * distance_squared(x.position, x1.position) /
                            distance_squared(x1.position, source_reservoir.org_vertex->position);
    p_hat = y.p_hat;
